...
## load data
cp.data <- Read10X(data.dir = "E:/苏州研究院/单细胞测序结果/result/data/surCP_matrix/")
cp <- CreateSeuratObject(counts = cp.data, project = "cystic")

## QC and select cells for further analysis
cp[["percent.mt"]] <- PercentageFeatureSet(cp, pattern = "^MT-")
cp[["percent.lg"]] <- PercentageFeatureSet(cp, pattern = "^IG")
# Visualize QC metrics as a violin plot
VlnPlot(cp, features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "percent.lg"), ncol = 4)
cp <- subset(cp, subset = nFeature_RNA > 200 & nFeature_RNA < 5000 & percent.mt < 70 & percent.lg < 50)
...
cp <- FindClusters(cp, resolution = 0.7)

## plot tsne
cp <- RunTSNE(cp, dims = 1:20)
...
write.csv(all.markers, "../table/SOLID&CYSTIC/diff_genes_cys.csv", row.names = F)
write.csv(top100, "../table/SOLID&CYSTIC/diff_genes_top100_cys.csv", row.names = F)


## cell annotation
library(ComplexHeatmap)
new.cluster.ids <- c("Distal absorptive", "Stem" ,"MT", "Distal absorptive","Enterocyte","B",
                     "Wnt signature", "Wnt signature", "TA","Paneth", "BEST4+")
names(new.cluster.ids) <- levels(cp)
cp <- RenameIdents(cp, new.cluster.ids)

## remove mt and B cells
cp = subset(cp, 
            idents = unique(Idents(cp))[!unique(Idents(cp))=="MT"])
cp = subset(cp, 
            idents = unique(Idents(cp))[!unique(Idents(cp))=="B"])

## tsne plot
DimPlot(cp, reduction = "tsne",
        cols = c("#8e787b","#b1c75f","#f2d900","#e6a1c6","#68b0d6", "#B36CC2", "#E27876"))

## tsne feature plot
FeaturePlot(cp, features =c("LGR5","OLFM4","EPHB2",  ## STEM
                            "FAM3D", ## Distal Absorptive
                            "FABP1",  ## Enterocyte
                            "MKI67","TOP2A",  # TA
                            "TCF4", "BMP4",   ## WNT 
                            "BEST4", "OTOP2", ## Best4+
...
select_gene = top50$gene[!grepl("^MT-", top50$gene)]
select_gene = select_gene[!grepl("^HLA-", select_gene)]
select_gene = select_gene[!grepl("^IG", select_gene)]

DoHeatmap(cp, features = select_gene,
          group.colors = c("#8e787b","#b1c75f","#f2d900","#e6a1c6","#68b0d6","#B36CC2","#E27876"),
          size = 2) + 
  scale_fill_gradientn(colours =c("navy","white","firebrick3"))

saveRDS(cp,"../RDS data/SOLID&CYSTIC/cystic.rds")
