...
solid.data <- Read10X(data.dir = "../../data/solid_Matrix/")
...
solid <- subset(solid, subset = nFeature_RNA > 200 & nFeature_RNA < 5000 & percent.mt < 75)
...
## Cluster the cells
solid <- FindNeighbors(solid, dims = 1:25, k.param = 70)
solid <- FindClusters(solid, resolution = 0.8)
## plot umap,tsne
# solid <- RunUMAP(solid, dims = 1:25)
# DimPlot(solid, reduction = "umap")
solid <- RunTSNE(solid, dims = 1:25)
...
write.csv(all.markers, "../../output/cell_identity/diff_genes_solid.csv", row.names = F)
write.csv(top100, "../../output/cell_identity/diff_genes_top100_solid.csv", row.names = F)
saveRDS(solid,"../../output/solid_cystic/solid.rds")

## cell annotation
library(ggplot2)
library(ComplexHeatmap)
# genes <- c("ASCL2","HELLES","PCNA","MKI67","TOP2A","KRT19","LYZ","TFF1","TFF3")
...
library(purrr)
library(fgsea)
gene_diff <- read.delim('../table/SOLID&CYSTIC/diff_genes_solid.csv', sep = ',', check.names = FALSE)
